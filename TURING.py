...
class Alphabet:
	"""On définit un alphabet comme un ensemble de lettres
        et un caractère blanc (epsilon)."""
	def __init__(self, lettres, epsilon):
		self.lettres = lettres
		self.epsilon = epsilon

class Ruban:
	"""On dédfinit un ruban comme un tableau
	unidimensionnel infini."""
	...
	def __getitem__(self, index):
		"""Retourne la valeur à l'index index du ruban. Si celle-ci n'a pas été
                définie, il s'agit de epsilon."""
...
class Machine:
	"""On définit une machine de Turing comme un ensemble de
	transitions et un ruban sur lequel la machine va agir."""
...
class Generator:
	...
	def faidestrucs(self, args):
		...
		if a == "DATA":
			...
			print(self.Vi[args[1]] - self.currentIndex)
			...
			if len(args) == 3:
				...
				self._get(args[1])
				self._generate_move(-self.n)
			self._generate_greater_or_equal(self.n)
		elif a == "LE":
			if len(args) == 3:
				self._get(args[1])
				self._generate_move(self.n)
				...
				self._get(args[1])
				self._generate_move(-self.n)
			self._generate_lesser_or_equal(self.n)
		elif a == "LT":
			if len(args) == 3:
				self._get(args[1])
				self._generate_move(self.n)
				...
				self._get(args[1])
				self._generate_move(-self.n)
			self._generate_lesser_than(self.n)
		elif a == "EQU":
			if len(args) == 3:
				self._get(args[1])
				self._generate_move(self.n)
	...
	def _generate_greater_or_equal(self, n):
		"""Relation d'ordre sur Z : >="""
		d = self.tr
		C = self.getConstante()
		startState, finalState = self.getStates()
		...
		d[(C + "M1", 0)] = (C + "N", 1, 0)
		d[(C + "M1", 1)] = (C + "A0", -n - 1, 1)
		d[(C + "A" + str(n - 1), 0)] = (C + "Y", 2 * n + 1, 0)
		d[(C + "A" + str(n - 1), 1)] = (C + "Y", 2 * n + 1, 1)
		d[(C + "Y", 0)] = (finalState, 0, 1)
		d[(C + "Y", 1)] = (finalState, 0, 1)
		d[(C + "N", 0)] = (finalState, 0, 0)
		d[(C + "N", 1)] = (finalState, 0, 0)
	...
	def _generate_lesser_than(self, n):
		"""Relation d'ordre sur Z : <"""
		d = self.tr
		C = self.getConstante()
		startState, finalState = self.getStates()
		...
		for k in range(n - 1):
			...
			d[(C + "A" + K, 1)] = (C + "B1(" + K + ")", n, 1)
			d[(C + "B0(" + K + ")", 0)] = (C + "A" + str(k + 1), -n - 1, 0)
			d[(C + "B0(" + K + ")", 1)] = (C + "Y", k + 2, 1)
			d[(C + "B1(" + K + ")", 0)] = (C + "N", k + 2, 0)
			d[(C + "B1(" + K + ")", 1)] = (C + "A" + str(k + 1), -n - 1, 1)
		self.currentIndex += (2 * n)

	def _generate_lesser_or_equal(self, n):
		"""Relation d'ordre sur Z : <="""
		d = self.tr
		C = self.getConstante()
		startState, finalState = self.getStates()
